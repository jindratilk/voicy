...
def main():
    ...
    app=a.app.resolve();out=a.output.resolve()
    if not app.is_dir() or app.name!='Voicy.app':p.error('Expected a Voicy.app directory')
    if out.exists():p.error('Output exists; preserve existing releases')
    mount=Path('/Volumes/Voicy')
    if mount.exists():p.error('Eject the existing Voicy volume first')
    run('codesign','--verify','--deep','--strict',str(app));run('xcrun','stapler','validate',str(app))
    with tempfile.TemporaryDirectory(prefix='voicy-dmg-') as tmp:
        tmp=Path(tmp);stage=tmp/'stage';stage.mkdir();run('cp','-cR',str(app),str(stage/'Voicy.app'));(stage/'Applications').symlink_to('/Applications');(stage/'.background').mkdir()
        im=Image.new('RGB',(1320,840),'#ececee');d=ImageDraw.Draw(im);font=lambda s:ImageFont.truetype('/System/Library/Fonts/SFNS.ttf',s)
        d.text((660,100),'voicy',font=font(58),fill='#171719',anchor='mm');d.text((660,171),'A clearer voice. One simple move.',font=font(25),fill='#77777d',anchor='mm');d.line((570,435,747,435),fill='#98989f',width=3);d.line((730,418,747,435,730,452),fill='#98989f',width=3);d.text((660,690),'Drag Voicy to Applications',font=font(25),fill='#44444b',anchor='mm');d.text((660,743),'Free. Local. Yours.',font=font(20),fill='#8a8a92',anchor='mm');im.resize((660,420),Image.Resampling.LANCZOS).save(stage/'.background/background.png')
