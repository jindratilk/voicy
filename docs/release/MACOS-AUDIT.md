...
## Exact-output runtime follow-up — 22 September 2026

The quality-preserving investigation is complete for this iteration. Three measured runs per variant gave 96.022 seconds for the original worker versus 95.282 seconds for the candidate (0.77% less time). Overlapping ranges mean this is not a meaningful universal speedup claim, and 10× was not achieved. All eight warmup/measured outputs were exactly equal. The app now reuses identical embeddings and applies RAM/headroom/pressure-based budgets; the approved model and 32-step settings remain unchanged. See the [final measurements and rejected experiments](INFERENCE-PERFORMANCE-AUDIT.md).
